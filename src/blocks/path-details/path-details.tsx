...
export class PathDetails {
  ...
  async updateState() {
    ...
    try {
      ...
      this.errorCode = (error instanceof NoiError) ? error.code : 'error.highway-time';
      this.loading = false;
    }
  }
  ...
  renderHighwayDetails() {
    if (this.errorCode) {
      return (
        <div class="last-sync">
          <p class="error">{translate(this.errorCode)}</p>
        </div>
      );
    }
    return (
  ...
  renderA22HeaderTab() {
    const highwayHeaderClass = {
      ...
      'header__section--err': !!this.errorCode
    };
    if (this.highwayTimeMin !== undefined) {
      return (
