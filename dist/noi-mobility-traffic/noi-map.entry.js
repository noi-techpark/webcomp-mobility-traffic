...
const NoiMap = class {
  ...
  componentDidLoad() {
    this.map = leafletSrc.map(this.el, { zoomControl: false });
    this.updateCenterAndZoom();
    leafletSrc.tileLayer(TILE_LAYER).addTo(this.map);
    this.renderChildren();
  ...
  entityAttrsObserver(el, mutationsList) {
    for (const mutation of mutationsList) {
      ...
      if (['class'].includes(mutation.attributeName)) {
        ...
        this.entityChildren.get(el).layer.setStyle(Object.assign(Object.assign({}, oldStyle), { className: el.getAttribute('class') }));
      }
    }
  }
  ...
  renderGeoJson(e) {
    ...
    const layer = leafletSrc.geoJSON(geometry, geometry);
