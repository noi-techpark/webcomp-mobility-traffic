...
function cancellablePromise(from) {
  ...
  const promise = new Promise((resolve, reject) => {
    from
      .then(d => isCancelled ? reject(isCancelled) : resolve(d))
      .catch(e => reject(isCancelled ? isCancelled : e));
  });
...
function setupPathStore(store, effect) {
  ...
  function loadUrbanPath(startId, endId) {
    set('loading', true);
    set('errorCode', undefined);
    const from = effect(startId, endId);
    effectPromise = cancellablePromise(from);
    effectPromise.then(path => {
      set('path', path);
    })
      .catch(err => {
      if (err instanceof NoiError) {
